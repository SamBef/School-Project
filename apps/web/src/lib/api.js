...
const BASE_URL = (import.meta.env.VITE_API_URL ?? '').replace('localhost:3004', 'localhost:3005');
...
function getConnectionHint(url) {
  ...
  if (isLocal) {
    return 'Make sure the API is running (e.g. run "npm run dev" in apps/api). Check apps/web/.env has VITE_API_URL=http://localhost:3005 (or the port your API uses).';
  }
...
async function request(path, options = {}) {
  if (!BASE_URL || !BASE_URL.startsWith('http')) {
    throw new Error(
      'API URL is not configured. Set VITE_API_URL in apps/web/.env for local dev (e.g. http://localhost:3005). For production, set it in your host (e.g. Vercel) to your API URL, then redeploy.'
    );
  }
